...
-- local ap_locations = require("AP_LocationIds")  -- generated by ExportLua
...
local function completeQuestCheck(player, questId)
    local loc = AP_QuestToLocation[questId]
    if not loc then
    ...
    if loc then
        local key = tostring(loc)
        local id = tostring(AP_LocationIds[key])
        AP_Checks[id] = true
        saveChecks()
    end
...
RegisterPlayerEvent(13, function(event, player, oldLevel)
    ...
    local loc = level_to_location[newLevel - 1]
    if loc then
        local key = tostring(loc)
        local id = tostring(AP_LocationIds[key])
        AP_Checks[id] = true
...
function CHECKS.completeSpellCheck(player, spellId)
    ...
    player:SaveToDB()
    local loc = AP_SpellToLocation[spellId]
    print(loc)
    if loc then
        local key = tostring(loc)
        local id = tostring(AP_LocationIds[key])
